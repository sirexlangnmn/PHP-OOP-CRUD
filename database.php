...
class Database
{
     ...
     */
    function __construct($database_name = 'native_crud', $username = "root", $password = "", $host = 'localhost')
    {
        self::$instance = $this;
        $this->database_name = $database_name;
        $this->mysql = mysqli_connect($host, $username, $password, $database_name);

        foreach($this->prepare_tables() as $key => $table){
            if(!$this->table_exists( $key )){
                $create = $this->mysql->query( $table );
            }
        }
        if (!$this->mysql) {
